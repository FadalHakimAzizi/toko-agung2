...
const HISTORY_TURNS = 10 // giliran percakapan terakhir yang disertakan sebagai konteks
...
export async function POST(request: Request) {
  try {
    ...
    const hasPriorUserTurn = history.some((h) => h.role === "user")
    ...
    // 1) Retrieval (dengan fallback ke topik percakapan sebelumnya jika perlu)
    const ctx = await retrieveContext(question, isAdmin, history)
    ...
    if (templateAnswer !== REFUSAL_MESSAGE || hasPriorUserTurn) {
